...
ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), data = fort_tas, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude), data= centroids, colour = "#b2df8a") + theme_void() + coord_equal()
```
...
```{r}
allocation <- centroid_allocation
hexmap_lga <- left_join(tas_lga, allocation)
ggplot() + 
...
hexmap_lga <- left_join(tas_lga, 
  create_hexmap(shp = tas_lga, sf_id = "LGA_CODE11", buffer_dist = NULL, filter_dist = 10, hex_size = 0.2, export_shp = FALSE, focal_points = sugaRbag::capital_citie, verbose = TRUE)
)
```
