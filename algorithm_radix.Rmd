...
author:
  - name: Stephanie Kobakian
    ...
    affiliation: nswmania University of Technology
date: "`r Sys.Date()`"
...
Spatial distrbutions have utilised alternative representations of geography for many years. In modern times with the ease of interactivity and animation, alternative representations have been popularised by online news sites. Applications are iincreasingly widespread, especially in the area of disease mapping.
...
This provides an opportunity for a new class of
...
knitr::kable(tibble::tibble(parameter = c('shp', 
  ...
  'sf_id',
  'focal_points'),
  description = c('a shape file', 'character string location of shape file', 'name of a unique column that distinguishes areas',
...
nsw_shp <- read_shape(shp_path = shp_path)

# Convert to a simple features object
nsw_lga <- sf::st_as_sf(nsw_shp)

# Select only desired LGA areas, those from nswmania.
nsw_lga <- nsw_shp %>% filter(STE_NAME11 == "New South Wales")
nsw_lga[[sf_id]] <- droplevels(as.factor(nsw_lga[[sf_id]]))

## Not necessary when using `create_hexmap`:
## Derive projection information
crs_info <- sf::st_crs(nsw_lga)
...
centroids <- create_centroids(shp_sf = nsw_lga, sf_id = sf_id, projstring = projstring, verbose = FALSE)
```


```{r, fig.cap="nswmania Local Government Areas and derived centroids."}
fort_nsw <- fortify_sfc(nsw_lga, projstring = projstring)
ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude), data= centroids, colour = "#b2df8a") + theme_void() + coord_equal()
```
...
```{r}
ggplot() + geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude), data= centroids, colour = "#b2df8a") + theme_void() + coord_equal() +
...
```{r}
ggplot() + geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude),data= centroids, fill = "#b2df8a") + theme_void() + coord_equal() +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#1f78b4", data = grid, size = 0.75) 
```
...
```{r}
ggplot() + geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude),data= centroids, fill = "#b2df8a", fill = "red") + theme_void() + coord_equal() +
...
Create a box filter
```{r, echo = FALSE}
ggplot() + geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude),data= centroid1, fill = "red") + theme_void() + coord_equal() +
...
Filter for angle within circle, relates hexagon location to focal point, Hobart.
...
ggplot(hex_points_df) + 
  geom_polygon(aes(x = hexv_long, y = hexv_lat, group = LGA_CODE11)) +
  coord_equal() +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#a6cee3", data = grid, size = 0.25) +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#1f78b4", data = hex_grid, size = 0.75)
```
...
```{r fullhexmap}
lga_map <- create_hexmap(shp = nsw_lga, sf_id = "LGA_CODE11", buffer_dist = NULL, filter_dist = 10, export_shp = FALSE, focal_points = sugaRbag::capital_cities, projstring = projstring, verbose = TRUE)

hexmap_lga <- left_join(nsw_lga, 
  lga_map)
...
```{r}
ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") +
...
Examples of Melbourne, nswmania, Australia
