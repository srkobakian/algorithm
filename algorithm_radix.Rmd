...
The polygon set of Local Government Areas in Australia in 2016 is stored in the sugarbag package data as `tas_lga`. To create this object:
...
knitr::kable(tibble(parameter = c('projstring', 
  'epsg', 'buffer_dist', 'hex_size',  'hex_filter', 'width'),
...
We will use the Statical Areas (Level 2) of Australia, provided by Australian Bureau of Statistics. 

```{r}
load("data/tas_lga.rda")

# Convert to a simple features object
tas_lga <- sf::st_as_sf(tas_lga)

# Select only desired LGA areas.
...
ggplot(hex_points_df) + 
  geom_polygon(aes(x = hexv_long, y = hexv_lat, group = LGA_CODE16)) +
  coord_equal() +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#a6cee3", data = grid, size = 0.25) +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#1f78b4", data = hex_grid, size = 0.75)
```
...
width = 30,
 verbose = TRUE) # same column used in create_centroids
```


```{r}
h1 <- hex_allocated %>%
  dplyr::group_nest(LGA_CODE16) %>%
                mutate(hex = purrr::map(data, fortify_hexagon, hex_size = 0.1)) %>%
                unnest(data) %>% 
  unnest(hex) %>% left_join(., tas_lga)

p1 <- fortify_sfc(tas_lga)

end_hex <- ggplot() +
  geom_polygon(data = p1, aes(x=long, lat, group = group), alpha = 0.1) +
  geom_polygon(data = h1, aes(x=hexv_long, hexv_lat, group = LGA_CODE16)) + theme_void() + coord_equal()
