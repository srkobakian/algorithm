...
# Select only desired SA2 areas, those from tas.
...
end_cents <- ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), data = fort_tas, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude), data= centroids) + theme_void() + coord_equal()
end_cents
...
ggplot(hex_points_df) + 
  geom_polygon(aes(x = hexv_long, y = hexv_lat, group = SA2_5DIG16)) +
  coord_equal() +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#a6cee3", data = grid, size = 0.25) +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#1f78b4", data = hex_grid, size = 0.75)
```
