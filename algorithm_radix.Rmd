...
The polygon set of Local Government Areas in Australia in 2011 is stored in the sugaRbag package as `lga_2011`.
...
We will use the Local Government Areas of Australia in the `lga_2011` data set, and take only those from New South Wales as an example:

```{r}
shp_sf <- sugaRbag::lga_2011

nsw_lga <- shp_sf %>% dplyr::filter(STE_NAME11 == "New South Wales")
# Select only desired LGA areas, those from nsw.
...
nsw_lga <- sf::st_transform(nsw_lga, crs = projstring)
```


```{r area_variable}
nsw_lga <- nsw_lga %>% mutate(logarea = log(AREA_SQKM))
```
...
centroids$logarea <- nsw_lga$logarea 
  
fort_nsw <- fortify_sfc(nsw_lga, projstring = projstring)
```


```{r, fig.cap="NSW Local Government Areas and derived centroids."}
fort_nsw <- fortify_sfc(nsw_lga, projstring = projstring)

end_cents <- ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), data = fort_nsw, fill = "grey", colour = "white") + geom_point(aes(x=longitude, y = latitude, colour = logarea), data= centroids) + theme_void() + coord_equal()
end_cents
```
...
Examples of Melbourne, nsw, Australia
