...
title: "sugarbag: An Algorithm for Transforming Spatial Polygons into a Hexagon Tile Map, Implemented in R"
description: >
  Maps of geographic areas are constructed from a set of spatial polygons. These are commonly used to create a choropleth map, where polygons are colored to indicate the value of some variable of interest. Information on small areas can get lost on the map, and a solution is to tesselate hexagons so that all areas are equal size. This paper describes a new algorithm to automatically construct a spatial layout of tesselated hexagons, where the hexagons don't need to  be contiguous. The result is called a hexagoon tile map, and the implementation is available in the R package, sugarbag. Applications of choropleth and hence HTM: atlases, ..
keywords:
...
The current practice for presenting geospatial data is a choropleth map display. These maps highlight the geographic patterns in geospatially related statistics \citep{SAMGIS}.
...
<!-- solutions -->
The solutions to this visualisation problem begin with the geography. Cartograms apply a transformation to the geographic boundaries based on the value of the statistic of interest. These displays result in a distortion of the map space to represent differences in the statistic across the areas \citep{ACCAC}. 
The statistic of interest is used to determine the cartogram layout, Cheng [@IVMVTSAD] reviews these methods discusses the shape errors that occurs when creating cartgorams. When the Australian population is the statistic of interest, the result is a population cartogram. They fail to preserve a recognisable display due to the difference in size of metropolitan and rural areas \citep{ACTUC}, \citep{GOINO}.
...
Tile maps, Rectangular cartograms \citep{ORC} and Dorling cartograms \citep{ACTUC}, all use one simple shape to represent each area. They place various importance on the preservation of spatial relationships, but all decrease the emphasis on the size of the geographic areas. These alternative map displays focus on the relationship between neighbours, attempting to preserve connections, and disregard the unique shapes of the administrative boundaries.

The \pkg{sugarbag} package provides a new algorithm to create tesselated hexagon tile maps. 
...
The algorithm presented in \pkg{sugarbag} package operates on a set of simple feature geometry objects, also known as \pkg{sf} \citep{sf} polygons.

There are four steps performed to create a tesselated hexagon tile map.
These steps can be executed by the main function, \code{create_hexmap}, or can be implemented separately for more flexibility. There are parameters used in the process that can be provided by users, if they are not, they will be automatically derived.
...
The following parameters will be determined within \code{create_hexmap} if they are not provided. They are created as they are needed throughout the following example:
...
A set of centroids may be used directly. The set of polygons should be provided as an \pkg{sf} object, this is a data frame containing a \code{geometry} column. The \code{read_shape} function can assist in creating this object for use in `R`.
...
```{r algorithm, out.width = "100%", echo = FALSE, fig.cap = "The grid of points used to create a hexagon tile map for the Australian state of Tasmania, at each step in the sugarbag algorithm. The steps in the create a hexagon tile map from the centroids of geographic shapes of the Statistical Areas of Tasmania at Level 2. The green points show the locations of the centroids of the SA2 areas. The blue dots show the grid points available for allocation at each buffer step.", fig.height = 12, fig.width = 9}
knitr::include_graphics(path = "figures/filter.png")
```
...
The grid of possible hexagon locations is made using the \code{create_grid} function.
...
A set of longitude columns, and latitude rows are created to define the locations of the hexagons. The distance between each row and column is the size specified by \code{hex_size}.
...
Figure \ref{fig:algorithm}a shows the original grid on the left, to allow for tessellating hexagons, every second latitude row on the grid is shifted right, by half of the hexagon size. The grid for tessellation is shown on the right in Figure \ref{fig:algorithm}b.
...
To filter the grid for appropriate hexagon locations for allocation, the \code{create_buffer} function is used by \code{create_grid}.
...
The amount of rows in each latitude group and the amount of columns in each longitude group are used as the width of rolling windows. The rolling windows can be seen on the bottom and right of the gris shown in Fig. \ref{fig:filter_grid}.
...
The \code{width} parameter is used to take a slice of the remaining points. The width is the amount of degrees used on either side of the angle from the focal point to centroid location. This uses the angle from the closest capital city, to the current centroid as seen in Figure \ref{fig:algorithm}c and \ref{fig:algorithm}d. This allows the spatial relationship to be preserved, even when it is allocated to a hexagon that is further from the focal point then the original centroid location.


<!-- ```{r angles, fig.cap = "Filter for grid points within the angle from the focal point to the centroid.", echo = FALSE, out.width = "100%"} -->
<!-- knitr::include_graphics(path = "figures/5allocate.png") -->
<!-- ``` -->
...
This requires specifying the \code{sf} object as the argument for the \code{use_neighbours} parameter. This calculates neighbours using intersections of their polygons. This occurs for all areas before any allocations begin.
...
It requires connecting the polygons for each area in two displays, which can be done using the \code{sf_id} variable, such as the SA2 name.
...
The \pkg{sugarbag} package creates tesselated hexagon tile maps. The Australian application preserves the spatial relationships, emphasising capital cities. The hexagon tile map is a visualisation solution that highlights spatial distributions.
...
The authors would like to thank the Australian Cancer Atlas team for discussions regarding alternative spatial visualizations. They would also like to thank Professor Kerrie Mengersen for suggestions and comments. Mitchell O'Hara-Wild provided assistance with some parts of the algorithm and code. Sayani Gupta provided helpful advice on parts of this paper. 
