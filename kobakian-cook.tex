...
The non-contiguous cartogram in figure \ref{fig:tas_displays}c) also
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=0.95\linewidth]{kobakian-cook_files/figure-latex/centroids_plot-1} \caption[The Australian Cancer Atlas data has determined the colour of each Statistical Area of Australian at Level 2]{The Australian Cancer Atlas data has determined the colour of each Statistical Area of Australian at Level 2. A choropleth map (a) of Standardised Incidence Rates (SIRs) is paired with a hexagon tile map (b) to contrast the colours that are made obvious when every SA2 is equally represented.}\label{fig:centroids_plot}
\end{figure}
\end{Schunk}
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=0.95\linewidth]{kobakian-cook_files/figure-latex/grid2-1} \caption[Complete set of grid points to create a tile map]{Complete set of grid points to create a tile map.}\label{fig:grid2}
\end{figure}
\end{Schunk}
...
grid shown in Figure \ref{fig:grid3}. This will tailor the available
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=0.95\linewidth]{kobakian-cook_files/figure-latex/grid3-1} \caption[The remaining hexagon locations from all the possible points in the initial grid after the buffer is applied]{The remaining hexagon locations from all the possible points in the initial grid after the buffer is applied. The blue dots show the grid points left to choose from after the buffer step.}\label{fig:grid3}
\end{figure}
\end{Schunk}
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=1\linewidth]{kobakian-cook_files/figure-latex/buffers-1} \caption[Filter for grid points within a square, then circular, distance for those closest to the centroid]{Filter for grid points within a square, then circular, distance for those closest to the centroid.}\label{fig:buffers}
\end{figure}
\end{Schunk}
...
\ref{fig:buffers2} . This allows the spatial relationship to be
preserved, even when it is allocated to a hexagon that is further from
the focal point then the original centroid location.

\begin{Schunk}
\begin{figure}
\includegraphics[width=1\linewidth]{kobakian-cook_files/figure-latex/buffers2-1} \caption[Filter for grid points within a square, then circular, distance for those closest to the centroid]{Filter for grid points within a square, then circular, distance for those closest to the centroid.}\label{fig:buffers2}
\end{figure}
\end{Schunk}
...
Cancer Atlas site. This small example in Figure \ref{fig:centroids_plot}
