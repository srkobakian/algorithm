...
disease mapping, and election results. The algorithm presented here
creates a display that uses tessellated hexagons to represent a set of
spatial polygons, and is implemented in the R package called sugarbag.
It allocates these hexagons in a manner that preserves the spatial
...
The Australian Cancer Atlas is an online, interactive display of
...
\CRANpkg{cartogram} \citep{cartogram} for R \citep{R}, promote better
...
algorithm available in the \CRANpkg{sugarbag} package. How to utilise
...
implemented in \pkg{cartogram} and tile maps implemented in
\CRANpkg{tilemaps} \citep{tilemaps}, all use one simple shape to
...
\CRANpkg{statebins} \citep{statebins}.The algorithm described below
...
objects , known as \CRANpkg{sf} objects \citep{sf}. This package allows
R users to create \pkg{sf} objects by importing polygons stored in
...
The user can tweak the parameters of the hexagon map using additional
...
than polygons, the \code{allocate} function can be used directly.

The set of SA2 polygons for Tasmania, using the 2011 definition of SA2s,
was accessed from the \pkg{absmapsdata} package \citep{absmapsdata}. A
...
Australia serves as a good use case for the algorithm as it represents
