...
aus_ggchoro <- ggplot(sa2thyroid_ERP) + 
  geom_sf(fill = "lightgrey", data = states %>% filter(state_name_2011=="Tasmania")) + 
...
aus_ggcont <- ggplot(cont) + 
  geom_sf(fill = "darkgrey",  data = states %>% st_transform(., 3112) %>% filter(state_name_2011 == "Tasmania")) +
...
# Hobart
hobart <- ggplot(ncont %>% filter(gcc_name_2011 == "Greater Hobart")) + 
  geom_sf(fill = "darkgrey",  data = sa2thyroid_ERP %>% filter(gcc_name_2011 == "Greater Hobart")) +
...
The choropleth map that uses the geographic shapes of areas can be seen in figure \ref{fig:melanoma-geo}. Figure \ref{fig:melanoma-hex} displays the Australian areas with equally sized hexagons. This display allows the small and densely populated inner city areas to be easily contrasted with their neighbouring areas.
