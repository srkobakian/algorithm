...
tas_centroids <- ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = interaction(sa2_name_2011, polygon)), data = fort_tas, fill = "#D3D3D3", colour = "white", size = 0.01) + 
...
```{r filtergrid, fig.cap = "The remaining hexagon locations from all the possible points in the initial grid after the buffer is applied. The blue dots show the grid points left to choose from after the buffer step."}
g1 <- ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = interaction(sa2_name_2011, polygon)), data = fort_tas, fill = "#D3D3D3", colour = "white", size = 0.01) + 
   theme_void() + 
...
g2 <- ggplot() + 
  ...
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#09661f", data = grid, size = 1.5) + ggtitle("b")
...
f1 <- ggplot() + 
  ...
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#09661f", data = f_grid, size = 1.5) + 
  geom_point(aes(x=longitude, y = latitude), data = f_centroid, fill = "#ede187", shape = 24) + 
#Hobart
  geom_point(aes(x = focal_longitude, y = focal_latitude), data = f_centroid, colour = "#e9f2ed", size = 2, shape = 3) + ggtitle("c") 

```

The `width` parameter is used to take a slice of the remaining points. The width is the amount of degrees used on either side of the angle from the focal point to centroid location. This uses the angle from the closest capital city, to the current centroid as seen in Figure \ref{fig:filterprocess} . This allows the spatial relationship to be preserved, even when it is allocated to a hexagon that is further from the focal point then the original centroid location.

```{r filterprocess, fig.cap = "Filter for grid points within a square, then circular, distance for those closest to the centroid.", echo = FALSE, out.width = "120%", height = 8}
...
f2 <- ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = interaction(sa2_name_2011, polygon)), data = fort_tas, fill = "#D3D3D3", colour = "white", size = 0.01) +
  geom_polygon(aes(x = long, y = lat, group = interaction(sa2_name_2011)), data = fort_allocated, fill = "#353b38", colour = "black", size = 0.01) +
  #Hobart
  geom_point(aes(x = focal_longitude, y = focal_latitude), data = f_centroid, colour = "#e9f2ed", size = 2, shape = 3) + 
  geom_point(aes(x=longitude, y = latitude), data = f_centroid, fill = "#ede187", shape = 24) + 
  theme_void() + coord_equal() +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#09661f", data = f_grid, size = 1) +
  geom_point(aes(x = hex_long, y = hex_lat), colour = "#e374b9", data = hex, size = 1.5) + ggtitle("d")
